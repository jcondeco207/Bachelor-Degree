...
public class PostgreInterface {
    ...
    PostgreInterface(String db, String username, String password){
        try {
    ...
    List<Ad>Search(String query){
        ...
        try{
            ...
            while(results.next()){
                ...
                Ad r = new Ad(location, price, gender, date, advertiser, typology, type, state, aid);
                r.setState(state);
                registry.add(r);
            }
    ...
    //select username from users where alias = (select advertiser from ads where aid = 2);
    ...
    // select alias from users where username = 'jrcondeco'
